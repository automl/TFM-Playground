...
if __name__ == "__main__":
    ...
    parser.add_argument("-model_type", type=str, choices=["regression", "classification"], required=True, help="Whether to use the regressor or classifier model")
    parser.add_argument("-checkpoint", type=str, default=None, help="Path to load the model weights from. If None, default weights are used.")
    parser.add_argument("-dist_path", type=str, default=None, help="Path to load the bucket edges for the support bar distribution from. Only needed for regression.")
    parser.add_argument("-tasks", type=str, default="tabarena-v0.1", choices=["tabarena-v0.1", "toy_tasks"], help="Which OpenML tasks to evaluate on.")
    parser.add_argument("-cache_directory", type=str, default=None, help="Directory to save OpenML data. If None, default cache path is used.")
    parser.add_argument("-max_n_features", type=int, default=500, help="Maximum number of features allowed for a task. Tasks exceeding this limit are skipped.")
    parser.add_argument("-max_n_samples", type=int, default=10_000, help="Maximum number of instances allowed for a task. Tasks exceeding this limit are skipped.")
    parser.add_argument("-num_mem_chunks", type=int, default=8, help="Chunks attention computation into `num_mem_chunks` many chunks to reduce memory usage.")
    ...
    predictions = get_openml_predictions(model=model, tasks=tasks, max_n_features=args.max_n_features, max_n_samples=args.max_n_samples, classification=(args.model_type == "classification"), cache_directory=args.cache_directory)
