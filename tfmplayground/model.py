...
class NanoTabPFNModel(nn.Module):
    ...
    def forward(self, *args, **kwargs) -> torch.Tensor:
        ...
        single_eval_pos is the length of X_train.
    ...
    def _forward(
        self, src: tuple[torch.Tensor, torch.Tensor], single_eval_pos: int, num_mem_chunks: int = 1
    ) -> torch.Tensor:
        x_src, y_src = src
...
class TransformerEncoderStack(nn.Module):
    def __init__(self, num_layers: int, embedding_size: int, num_attention_heads: int, mlp_hidden_size: int):
        """
        Instantiates num_layers many Transformer Blocks and stores them in a list so we can use
        them in the forward
        """
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, single_eval_position: int, num_mem_chunks: int = 1) -> torch.Tensor:
        ...
        Args:
            x: (torch.Tensor) a tensor of shape (batch_size, num_rows, num_features, embedding_size)
